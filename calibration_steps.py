...
def ap_corr(row_of_df, start_dither):
	...
	for alf in alf_files:
		...
		alf = alf.replace('.alf', '.alf_loc')
		als = alf.replace('.alf_loc', '.als_loc')
		ap = alf.replace('.alf_loc', '.ap_loc')


		# Image name
		image = alf.replace('.alf_loc', '.fits')

		# lst file name
		lst = alf.replace('.alf_loc', '.lst')
		...
		df_als = pd.read_csv(als, skiprows=1, header=None, names=['ID_als', 'X_als', 'Y_als', 'psf_mag', 'psf_error'], delim_whitespace=True)
		...
		new_filename = alf.replace('.alf_loc', '.alf_apc')
...
def zp_and_std_ape(row_of_df, start_dither):
	...
	for alf in alf_files:
		...
		new_filename = alf.replace('.alf_apc', '.alf_cal')
